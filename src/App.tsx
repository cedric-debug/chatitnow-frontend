...
export default function ChatItNow() {
  ...
  useLayoutEffect(() => {
    ...
    // UPDATED COLORS
    const DARK_BG = '#272C35'; // The specific blue-gray requested
        ...
        <div className={`absolute top-[60px] bottom-[60px] left-0 right-0 overflow-y-auto p-2 pb-4 space-y-3 z-10 ${darkMode ? 'bg-[#272C35]' : 'bg-white'}`}>
                        ...
                        <div className={`flex flex-col ${msg.type === 'you' ? 'items-end' : 'items-start'} relative`}>
                          ...
                          <div className={`relative px-3 py-2 rounded-2xl text-[15px] shadow-sm leading-snug ${
                            msg.type === 'you'
                              ...
                              : `${darkMode ? 'bg-[#2A303C] text-gray-100' : 'bg-gray-100 text-gray-900'} rounded-bl-none`
                          }`}>
                            {msg.text}
                            {msg.timestamp && (
                            ...
                            <div className={`absolute -bottom-2 ${msg.type === 'you' ? '-left-2' : '-right-2'} flex gap-[-5px]`}>
                              {msg.reactions?.you && (
                                <div className={`text-sm bg-gray-100 dark:bg-[#323846] border dark:border-[#4B5563] border-gray-300 rounded-full w-6 h-6 flex items-center justify-center shadow-sm z-20`}>
                                  {msg.reactions.you}
                                </div>
                              )}
                              {msg.reactions?.stranger && (
                                <div className={`text-sm bg-gray-100 dark:bg-[#323846] border dark:border-[#4B5563] border-gray-300 rounded-full w-6 h-6 flex items-center justify-center shadow-sm z-10 -ml-2`}>
                                  {msg.reactions.stranger}
                                </div>
                              )}
                            </div>
          ...
          {isTyping && (
            <div className="flex justify-start w-full">
              <div className={`${darkMode ? 'bg-[#2A303C]' : 'bg-gray-100'} px-3 py-2 rounded-2xl rounded-bl-none shadow-sm border-0 flex items-center`}>
        ...
        <div className={`absolute bottom-0 left-0 right-0 p-2 border-t z-20 flex flex-col justify-end ${darkMode ? 'bg-[#272C35] border-[#323844]' : 'bg-white border-gray-100'}`}>
          ...
          {replyingTo && (
            <div className={`flex items-center justify-between px-4 py-2 mb-1 rounded-lg text-xs border-l-4 ${darkMode ? 'bg-[#323844] text-gray-300 border-purple-500' : 'bg-gray-100 text-gray-700 border-purple-500'}`}>
              ...
              <button onClick={() => setReplyingTo(null)} className="p-1 hover:bg-gray-200 dark:hover:bg-[#3A4250] rounded-full">
                <X size={14} />
              </button>
            </div>
          )}
          ...
          {partnerStatus === 'disconnected' ? (
            ...
            <form className="flex gap-2 items-center h-[60px]" onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }}>
              <button type="button" onClick={handleNext} disabled={partnerStatus === 'searching'} className={`h-full px-3 w-16 rounded-xl flex items-center justify-center border-2 font-bold transition ${darkMode ? 'border-[#3A4250] text-white hover:bg-[#323844]' : 'border-gray-200 text-black hover:bg-gray-50 bg-white'} disabled:opacity-50`}>Skip</button>
              
              <input type="text" value={currentMessage} onChange={handleTyping} enterKeyHint="send" placeholder={isConnected ? (replyingTo ? `Replying to ${replyingTo.name}...` : "Say something...") : "Waiting..."} disabled={!isConnected} className={`flex-1 h-full px-3 rounded-xl border-2 focus:border-purple-500 outline-none transition text-[15px] ${darkMode ? 'bg-[#323844] border-[#3A4250] text-white placeholder-gray-400' : 'bg-white border-gray-200 text-gray-900'}`} />
